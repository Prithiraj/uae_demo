...
def primary_copy(r, theme):
    ...
    return f'{c} in {area}. This prototype keeps the verified phone, location, hours and public listing context easy to find.'
...
def render(r):
    ...
    photo_notice=''
    if photo.get('url'):
        qualifier='This photo is associated with another Dubai branch of the same business/brand, not confirmed as this exact branch.' if photo.get('scope')=='branch' else 'This image comes from a public business/listing source and is used only for this prototype.'
    ...
    section_intro={
      ...
      'food':'Food pages should earn appetite with real photography and real menu facts. Where a menu or price is not verified, this prototype does not invent one.',
...
<main id="main">
  ...
  <section class="story section">
    ...
    <div class="story-card alt reveal"><p class="eyebrow">Public proof</p><h2>{esc(rep or 'Listing identity verified')}</h2><p>Google Maps is used as the stable identity anchor for this prototype. Ratings, review counts and hours are point-in-time data and should be rechecked before launch.</p></div>
  </section>
  ...
  <section id="proof" class="section provenance">
    <div class="section-heading reveal"><p class="eyebrow">Prototype provenance</p><h2>Evidence-first by design.</h2></div>
    <div class="provenance-grid reveal">
      <p>This site is an implementation prototype built from public research dated <strong>4 September 2026</strong>. It intentionally does not invent prices, policies, services, inventory, testimonials or booking workflows.</p>
      <div class="source-links">{source_links}</div>
    </div>
    {photo_notice}
  </section>
</main>
<footer><span>{esc(name)} · Prototype {esc(r['code'])}</span><a href="{esc(r['maps_url'])}" target="_blank" rel="noopener">Google Maps ↗</a></footer>
