...
import re
p=Path('business-research/06AS-abdul-razzaq-mohiddin-abdulla-trading-establishment-al-quoz-industrial-area-1-br.md')
t=p.read_text(encoding='utf-8')
def sec(n,nextn,body):
    global t
    t=re.sub(rf'## {n}\. .*?(?=\n## {nextn}\.)',body.rstrip()+'\n',t,flags=re.S)
sec(2,3,"""## 2. Audience
...
sec(8,9,"""## 8. Information architecture
...
sec(9,10,"""## 9. Section-by-section layout
...
t=t.replace('**JSON-LD:** use `LocalBusiness (use a more specific tailoring type only if supported by the chosen schema vocabulary)`','**JSON-LD:** use `WholesaleStore` where appropriate and supported, otherwise `LocalBusiness`')
